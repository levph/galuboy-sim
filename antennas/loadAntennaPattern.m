...
%   Returns a struct:
%     pat.name          char
%     pat.is_isotropic  logical
%     pat.flat_gain_dbi numeric  (only for isotropic patterns)
...
%     - The CSV grid must be RECTANGULAR (every az repeats the same el list).
...
%   Comment lines start with '#'.
    ...
    % --- Regular grid -----------------------------------------------------
    % Expect header az_deg,el_deg,gain_dbi (case-insensitive).
    header = strsplit(lower(strtrim(data_lines{1})), ',');
    expected = {'az_deg', 'el_deg', 'gain_dbi'};
    if ~isequal(header, expected)
        error('loadAntennaPattern:badHeader', ...
              ['Expected header "az_deg,el_deg,gain_dbi" in %s, got "%s".\n' ...
               'Use the "isotropic,<gain>" sentinel for omni patterns.'], ...
              csv_path, data_lines{1});
    end
